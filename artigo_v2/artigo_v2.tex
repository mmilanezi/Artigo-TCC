...
Em adição ao modelo preditivo, o sistema incorpora uma \textbf{camada de
...
\textbf{Apto para Voo~--- Requer Atenção}: vento acima de 10 m/s
...
\textbf{Nível L2} identifica condições críticas e força o status \textbf{Não Apto para Voo}
...
Adicionalmente, a camada emite o status \textbf{Inconclusivo~--- Consulte Especialista}
em três situações: (a)~\textbf{baixa confiança} --- a probabilidade máxima entre
...
claro; (b)~\textbf{contradição detectada} --- a Classe~2 (Acidente) é predita com
probabilidade superior a 60\% em um cenário onde todas as condições meteorológicas
são benignas, sugerindo anomalia ou viés residual de amostragem; e (c)~\textbf{dados
...
\begin{itemize}
  ...
  (fusão metodológica das antigas categorias Incidente e Incidente Grave);
...
operacional, em vez de uma classificação binária simplificada. A fusão das antigas
categorias Incidente e Incidente Grave em uma única Classe~1 foi adotada por
critério metodológico: a fronteira entre elas é tênue nos dados do CENIPA e a
separação forçava o modelo a distinguir eventos com padrões muito similares,
prejudicando a generalização. O banco CENIPA contém, no período 2007--2025,
...
perigoso, foi introduzida a \textbf{Classe 0, Voo Normal}, representada por
...
aeronave representativa da aviação civil brasileira. A opção pela geração
sintética justifica-se pela inexistência de uma base pública equivalente ao
CENIPA para operações sem ocorrência na aviação civil brasileira.
...
foram transformadas pelo \textbf{OneHotEncoder}, que converte cada categoria em
...
\textbf{SMOTE} (\textit{Synthetic Minority Over-sampling Technique}), proposta
...
treino balanceado. Os hiperparâmetros de cada modelo foram definidos por
combinação de busca em grade (\textit{grid search})\footnote{\textit{Grid
search} (busca em grade): método de otimização de hiperparâmetros que avalia
sistematicamente todas as combinações de valores candidatos pré-definidos,
selecionando a configuração com melhor desempenho segundo uma métrica de
validação cruzada.} e avaliação experimental,
sendo os valores finais documentados na Seção~3.2. Para garantir que as
predições utilizadas no treinamento do meta-aprendiz fossem não enviesadas,
empregou-se a técnica de \textbf{predições \textit{out-of-fold}}\footnote{%
...
com \textbf{validação cruzada de 5 partições}
...
como \textbf{critério primário de seleção} do modelo final.
...
\begin{table}[ht]
  ...
  \begin{tabular}{lc}
    ...
    ROC-AUC               & 0,9842    \\
    Tempo de treinamento  & $\approx$7 min (403 s) \\
    \bottomrule
  \end{tabular}
  \fonte{Elaborado pelo autor.}
\end{table}
...
conjunto de teste (140 amostras), obteve F1 = 0,78. A fusão das antigas categorias
Incidente e Incidente Grave em uma única Classe~1 contribuiu para a melhoria do
desempenho nessa categoria, ao aumentar sua representatividade e homogeneidade no
conjunto de treinamento. Esse padrão é descrito por \citeonline{HARMS2020} como
característico de modelos de ML em dados aeronáuticos: eventos raros tendem a
apresentar desempenho substancialmente inferior, dada a escassez de padrões
...
As Figuras~\ref{fig:status_a} a \ref{fig:status_d} ilustram os quatro
status operacionais apresentados ao usuário pela interface do sistema, evidenciando
como as diferentes combinações de predição do modelo e das regras de
\textit{override} de segurança se traduzem em diagnósticos distintos.

\clearpage
A Figura~\ref{fig:status_a} ilustra o status \textbf{Apto para Voo~--- Sem Risco Identificado}: o modelo atribuiu
99,8\% de probabilidade à Classe~0 (Voo Normal) e nenhuma condição L1 ou L2 foi
identificada nas variáveis meteorológicas de entrada, resultando na classificação mais
favorável do sistema.
...
A Figura~\ref{fig:status_b} apresenta o status \textbf{Apto para Voo~--- Requer Atenção}: embora o
modelo tenha atribuído 75,6\% de probabilidade ao Voo Normal, a camada de
\textit{overrides} identificou condição L1 ativa, umidade de 97\% com formação de
nevoeiro e vento de 1,1 m/s, elevando automaticamente o status para atenção.
...
\clearpage
A Figura~\ref{fig:status_c} demonstra o status \textbf{Não Apto para Voo}: o modelo distribuiu
as probabilidades em 19,4\% para Voo Normal, 53,1\% para Incidente e 19,6\% para
Acidente, e a camada L2 detectou precipitação intensa de 21,4 mm/h, forçando a
classificação mais restritiva independentemente da predição do classificador.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.70\textwidth]{images/Exemplo_nao_apto.png}
...
\clearpage
A Figura~\ref{fig:status_d} exemplifica o status \textbf{Inconclusivo~--- Consulte Especialista}:
a ausência de dados meteorológicos históricos para o período e localidade consultados
impediu a construção completa do vetor de \textit{features}, resultando em um diagnóstico
indeterminado pelo sistema.
...
\subsection{Evolução do modelo: de quatro para três classes}

O modelo atual representa uma evolução metodológica em relação à versão anterior (v1.6),
que operava com quatro classes e sem separação formal de conjunto de teste independente.
Três decisões metodológicas simultâneas explicam o ganho de desempenho observado.

\textbf{Fusão de classes: de quatro para três.}
A versão v1.6 operava com as classes Voo Normal, Incidente, Incidente Grave e Acidente.
A análise do desempenho individual revelou que a classe Incidente Grave apresentava
F1-Score de 0,31 --- com precisão de 0,35, o que significa que 65\% das predições
nessa classe correspondiam a falsos positivos.
...
A avaliação da versão v1.6 era conduzida sobre dados que participaram do processo
...
O modelo atual adota o \textit{split} 80/15/5: 909 amostras (5\%) foram separadas
...
\textbf{Fase 4 como diagnóstico.}
Na versão v1.6, a Fase~4 selecionava um único ``modelo vencedor'' entre os quatro candidatos.
No modelo atual, os quatro modelos base são tratados como complementares e todos
alimentam o Stacking Ensemble, sendo a Fase~4 redefinida como etapa de diagnóstico
sobre o conjunto de validação (15\%) \cite{WOLPERT1992}.
A avaliação definitiva ocorre exclusivamente na Fase~5, sobre o conjunto de teste (5\%).

A Tabela~\ref{tab:comparativo} apresenta as métricas globais do Stacking Ensemble
comparadas entre as versões v1.6 e o modelo atual.

\begin{table}[ht]
  \caption{Comparativo de métricas globais do Stacking Ensemble: versão v1.6 vs.\ modelo atual}
  \label{tab:comparativo}
  \centering
  \begin{tabular}{lccc}
    \toprule
    \textbf{Métrica} & \textbf{v1.6 (4 classes)} & \textbf{Atual (3 classes)} & \textbf{Variação} \\
    \midrule
...
na versão v1.6 penalizava desproporcionalmente a média macro.
A Tabela~\ref{tab:comparativo_classes} compara o F1 por classe entre as duas versões.

\begin{table}[ht]
  \caption{Comparativo de F1-Score por classe: versão v1.6 vs.\ modelo atual}
  \label{tab:comparativo_classes}
  \centering
  \begin{tabular}{llcc}
    \toprule
    \textbf{Classe} & \textbf{Rótulo} & \textbf{v1.6} & \textbf{Atual} \\
    \midrule
...
entre Acidente e Incidente Grave recorrentes na versão anterior, o classificador
passou a discriminar melhor os acidentes reais.
O ganho de desempenho não é resultado de ajuste fino de hiperparâmetros,
mas das três decisões metodológicas descritas --- todas embasadas na literatura
