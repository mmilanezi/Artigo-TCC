...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.95\textwidth]{images/Modelo ML.JPG}
...
\textbf{Apto Requer Atenção}: vento acima de 10 m/s (RBAC 121, Emenda 07, Art.~121.429(a)) \cite{RBAC121},
rajada acima de 12,5 m/s \cite{ICAO2018},
precipitação moderada acima de 2,5 mm/h (Glossário ANAC/DECEA), umidade acima de
90\% com formação de nevoeiro (ICAO, Anexo~3) \cite{ICAO2018,WMO2025},
...
independentemente da predição: vento acima de 17 m/s (ICAO, Anexo~3, Apêndice~2) \cite{ICAO2018}, rajada acima de 25 m/s,
...
Essa estrutura hierárquica de gravidade garante que o modelo aprenda gradações
reais de risco operacional, em vez de uma classificação binária simplificada.
O banco CENIPA contém, no período 2007--2025, aproximadamente 13.185 ocorrências
rotuladas (Classes 1, 2 e 3) após integração meteorológica e remoção de registros
com variáveis críticas ausentes.
...
sintética e limpeza, foi dividido em treino (80\%, 14.536 amostras) e teste
(20\%, 3.635 amostras). A divisão foi estratificada por classe para manter a
distribuição original de gravidade em ambos os subconjuntos, procedimento
...
treinamento permaneceu desbalanceado, com a Classe~2 (Incidente Grave) sendo a
...
todas as quatro classes foram elevadas para 7.304 amostras no conjunto de
treino, totalizando 29.216 amostras balanceadas para o treinamento dos modelos
base. A Figura~\ref{fig:fase_dados} ilustra o fluxo completo de processamento
dos dados brutos até o conjunto balanceado pronto para treinamento.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{images/Fase de dados.JPG}
...
\begin{table}[ht]
  ...
  \begin{tabular}{lc}
    ...
    ROC-AUC               & 0,9507    \\
    Tempo de treinamento  & $\approx$17 min (991 s) \\
    \bottomrule
  \end{tabular}
  \fonte{Elaborado pelo autor.}
\end{table}
...
cujas importâncias no Random Forest confirmam empiricamente a relevância
das variáveis meteorológicas para a predição de gravidade; e (iii)~a presença da camada de \textit{overrides} de segurança de dois níveis,
...
\clearpage
A Figura~\ref{fig:status_a} ilustra o status \textbf{Apto Seguro}: o modelo atribuiu
...
\begin{figure}[H]
  ...
  \caption{Status operacional~(a): Apto Seguro (Classe~0, confiança $\geq$ 90\%)}
  \label{fig:status_a}
  \fonte{Elaborado pelo autor.}
\end{figure}

A Figura~\ref{fig:status_b} apresenta o status \textbf{Apto Requer Atenção}: embora o
...
\begin{figure}[H]
  ...
  \caption{Status operacional~(b): Apto Requer Atenção (condição L1 ativa ou Classe~1)}
  \label{fig:status_b}
  \fonte{Elaborado pelo autor.}
\end{figure}

\clearpage
A Figura~\ref{fig:status_c} demonstra o status \textbf{Não Apto}: o modelo distribuiu
...
\begin{figure}[H]
  ...
  \caption{Status operacional~(c): Não Apto (condição L2 ativa, Classe~2 ou Classe~3)}
  \label{fig:status_c}
  \fonte{Elaborado pelo autor.}
\end{figure}

\clearpage
A Figura~\ref{fig:status_d} exemplifica o status \textbf{Inconclusivo}: a ausência de
dados meteorológicos históricos para o período e localidade consultados impediu a
construção completa do vetor de \textit{features}, resultando em um diagnóstico
indeterminado pelo sistema.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{images/Exemplo_inconclusivo.png}
  \caption{Status operacional~(d): Inconclusivo (baixa confiança ou contradição meteorológica)}
  \label{fig:status_d}
  \fonte{Elaborado pelo autor.}
\end{figure}
