...
simples, os modelos base, chamados de \textit{nível 0} ou \textit{base learners}, são treinados de forma
...
incidentes da aviação civil brasileira entre 2007 e 2025. Cada ocorrência foi
...
(arquivos CSV por ano e estado, cobrindo o período de 2000 a 31 de março de 2026),
integrados às ocorrências do CENIPA por data e estado de origem. Para análises
de voos com datas posteriores a 31 de março de 2026 ou futuras (até 15 dias à
frente da data atual), o sistema consulta a API Open-Meteo \cite{OPENMETEO2026} em tempo de inferência.
A base de aeródromos brasileiros foi obtida do OurAirports \cite{OURAIRPORTS2026}, permitindo o
...
\cite{SCIKITLEARN2026} --- que transforma cada variável para média zero e desvio padrão unitário,
...
\citeonline{HARMS2020} demonstra que o \textit{oversampling}\footnote{%
\textit{Oversampling} (superamostragem): técnica de balanceamento de classes
que aumenta artificialmente o número de amostras das classes minoritárias,
gerando novas instâncias sintéticas para reduzir o desequilíbrio no conjunto
de treinamento.} é etapa metodológica essencial em dados aeronáuticos desbalanceados: sem ele, o modelo
...
A Figura~\ref{fig:fase_dados} ilustra o fluxo de processamento dos dados brutos
até o conjunto pronto para treinamento.
...
treino balanceado. Para garantir que as predições utilizadas no treinamento do
meta-aprendiz fossem não enviesadas, empregou-se a técnica de
\textbf{predições \textit{out-of-fold}} com \textbf{validação cruzada de 5
partições} (\textit{5-fold cross-validation}): o conjunto de treinamento é
dividido em 5 subconjuntos (\textit{folds}), e para cada \textit{fold}, os
modelos base são treinados nos 4 subconjuntos restantes e geram predições para
o subconjunto reservado, de modo que cada instância recebe uma predição de
um modelo que nunca a viu durante o treino. Essas predições fora da amostra
constituem as \textit{meta-features} que alimentam o meta-aprendiz. Após a
geração das meta-features, a Regressão Logística Multinomial meta-aprendiz foi
treinada sobre elas, e o ensemble completo foi serializado para uso em
inferência. A avaliação dos modelos empregou como critérios primários o
F1-Score Macro e o ROC-AUC, privilegiando o equilíbrio entre desempenho geral
e representatividade das classes minoritárias.
